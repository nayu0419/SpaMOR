...
def calculate_adj_matrix(adata,x="array_row",y="array_col"):
    x = adata.obs[x]
    y = adata.obs[y]
...
class ZIP(NB):  # 假设 NB 是基础类
    def __init__(self, pi, ridge_lambda=0.0, **kwargs):
        super().__init__(**kwargs)
        self.pi = pi  # 零膨胀概率
        self.ridge_lambda = ridge_lambda  # L2正则化系数
    def loss(self, y_true, y_pred, mean=True):
        scale_factor = self.scale_factor  # 假设这个是在基类中定义的
        eps = self.eps  # 防止数值不稳定
        y_pred = y_pred * scale_factor  # 调整预测值
        # 泊松分布的 log likelihood
        poisson_case = -torch.lgamma(y_true + 1.0) + (y_true * torch.log(y_pred + eps)) - y_pred
        # 处理零的情况
        zero_poisson = torch.exp(-y_pred)  # P(Y=0|lambda) for Poisson
        zero_case = -torch.log(self.pi + (1.0 - self.pi) * zero_poisson + eps)  # ZIP分布下的零值损失
        # 损失计算，根据y_true是否为0，选择不同的分支
        result = torch.where(torch.lt(y_true, 1e-8), zero_case, poisson_case)
        # L2正则化项
        ridge = self.ridge_lambda * torch.square(self.pi)
        result += ridge
        # 如果需要返回均值损失
